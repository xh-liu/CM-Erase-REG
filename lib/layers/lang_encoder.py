...
class RNNEncoderImgGlove(nn.Module):
  def __init__(self, dict_emb, vocab_size, word_embedding_size, word_vec_size, hidden_size, bidirectional=False, input_dropout_p=0, dropout_p=0, n_layers=1, rnn_type='lstm', variable_lengths=True):
  ...
  def forward(self, input_labels, img):
    ...
    if self.variable_lengths:
      ...
      # make ixs
      input_lengths_list = (input_lengths+1).data.cpu().numpy().tolist() # add img features to the list of words
      sorted_input_lengths_list = np.sort(input_lengths_list)[::-1].tolist() # list of sorted input_lengths
      sort_ixs = np.argsort(input_lengths_list)[::-1].tolist() # list of int sort_ixs, descending
      s2r = {s: r for r, s in enumerate(sort_ixs)} # O(n)
      recover_ixs = [s2r[s] for s in range(len(input_lengths_list))]  # list of int recover ixs
    ...
    # embed
    embedded = torch.cat([self.embedding1(input_labels), self.embedding2(input_labels)], 2)  # (n, seq_len, word_embedding_size)
    embedded = self.input_dropout(embedded)  # (n, seq_len, word_embedding_size)
    embedded = self.mlp(embedded)            # (n, seq_len, word_vec_size)
